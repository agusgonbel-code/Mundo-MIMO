...
test('home and parent gate meet child touch and labelling baseline',async({page})=>{await setAge(page,3);await assertTargets(page,'#home');await assertNames(page,'#home');await page.locator('[data-action="parent"]').first().click();await expect(page.locator('#parentGate')).toHaveClass(/on/);await assertTargets(page,'#parentGate');await expect(page.locator('#gateAnswer')).toHaveAttribute('aria-label',/Respuesta/)});
